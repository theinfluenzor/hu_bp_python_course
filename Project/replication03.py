...
class Replication(Process):
    ...
    def update(self, model):
        ...
        #in anticlock_Richtung
            if self.PolymeraseIII_anticlock.bound == False:
        ...
        #in clock_Richtung
            if self.PolymeraseIII_clock.bound == False:
        ...
        #wenn beide Richtungen komplett sind, wird double auf True gesetzt, um weitere Replikation zu verhindern
            if self.PolymeraseIII_anticlock.position >= self.DNA.length and (self.PolymeraseIII_clock.position >= self.DNA.length):
                self.double = True
        print self.Helicase_anticlock.position, self. PolymeraseIII_anticlock.position, self.Helicase_clock.position, self. PolymeraseIII_clock.position
    ...
    def terminate(self,DNA, Pol, Hel):
        ...
        if PolymeraseIII.bound == True:
            if self.Nucleotide >= 200 and (PolymeraseIII.position+100 <=self.DNA.length):
                PolymeraseIII.position += 100
                self.Nucleotide -= 200
            elif self.Nucleotide>1  and (PolymeraseIII.position+(self.Nucleotide/2) <=self.DNA.length):
                PolymeraseIII.position += self.Nucleotide/2
                self.Nucleotide -= 2*self.Nucleotide/2
